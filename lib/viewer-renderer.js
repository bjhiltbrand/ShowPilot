...
function renderTemplate(template, state) {
  ...
  // {NEXT_PLAYLIST} — context-aware substitution.
  // Elements inside a jukebox container should show the first pending queue entry (matching
  // rf-compat.js client behaviour) rather than nextScheduled, which falls back to the FPP
  // main-playlist next song when the queue is empty. Showing the FPP fallback inside the
  // jukebox section is misleading and causes a flash-then-disappear on page load because
  // rf-compat replaces it with '—' on the first refreshState() call.
  // We detect jukebox context by checking the raw template string (container markers have not
  // been expanded yet at this point — they're still literal placeholder strings).
  const firstQueued = (state.queue || [])[0];
  const nextJukeboxDisplay = firstQueued
    ? (state.sequences.find(s => s.name === firstQueued.sequence_name)?.display_name || firstQueued.sequence_name)
    : '—';
  function substituteNextPlaylist(str, outsideVal, insideVal, marker) {
    if (!str.includes(marker) || !str.includes('{NEXT_PLAYLIST}')) return str;
    return str.split(marker).map((seg, i) => {
      const val = (i % 2 === 1) ? insideVal : outsideVal;
      return seg.split('{NEXT_PLAYLIST}').join(
        `<span data-showpilot-next>${escapeHtml(val)}</span>`
      );
    }).join(marker);
  }
  html = substituteNextPlaylist(html, nextDisplay, nextJukeboxDisplay, '{jukebox-dynamic-container}');
  html = substituteNextPlaylist(html, nextDisplay, nextJukeboxDisplay, '{playlist-standard-dynamic-container}');
  // Catch any remaining {NEXT_PLAYLIST} in templates that use neither container marker:
...
.race-instructions p { margin: 0; }
  ...
  <script src="/rf-compat.js?v=73"></script>
  `;
