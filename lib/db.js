...
-- Race mode taps (v0.33.155+). No uniqueness constraint — one viewer
-- can tap the same sequence multiple times. Cleared at the start of
-- each new race round.
CREATE TABLE IF NOT EXISTS race_taps (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  sequence_name TEXT NOT NULL,
  viewer_token TEXT NOT NULL,
  tapped_at DATETIME DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS idx_race_taps_seq ON race_taps(sequence_name);
...
const configMigrations = [
  ...
  // cooldown_suppress_fpp_playlist (v0.33.152+): when 1, sequences in cooldown
  // are disabled in FPP's playlist JSON so they're skipped in normal rotation.
  // Requires plugin v0.13.40+. Default 0 — opt-in only.
  ['cooldown_suppress_fpp_playlist', 'INTEGER DEFAULT 0'],
  ...
  // Race mode (v0.33.155+): a tap-as-fast-as-you-can competitive voting mode.
  // One viewer can tap the same song many times. First to target wins (or
  // highest taps when the timer expires, if no target is set).
  ['race_duration_seconds',   'INTEGER DEFAULT 60'],   // 0 = end-of-current-sequence
  ...
  ['race_ends_at',            'TEXT'],                 // ISO timestamp race will end (null if end-on-song-end)
  ['race_winner',             'TEXT'],                 // sequence_name of winner (null until decided)
];
...
// Migrations for now_playing
if (!columnExists('now_playing', 'baseline_next_sequence_name')) {
  db.exec(`ALTER TABLE now_playing ADD COLUMN baseline_next_sequence_name TEXT`);
}
...
// Migration: multi-language audio variants (v0.33.162+, schema fix v0.33.172+)
//
// The original migration added a `language` column via ALTER TABLE but kept
// `hash TEXT PRIMARY KEY` — meaning only one row per hash was possible, making
// it impossible to store a language variant that shares a hash with the default
// track (or any other variant). The correct schema uses an autoincrement `id`
// as the primary key and a UNIQUE constraint on (media_name, language) so
// multiple language rows can share a hash freely.
//
// Migration strategy: rebuild the table if it still has `hash` as its primary
// key. SQLite doesn't support ALTER TABLE to change the primary key, so we do
// the standard rename-copy-drop dance inside a transaction.
{
  // Detect whether the old schema is still in place by checking if `id` column
  // exists — the new schema adds it; the old schema doesn't have it.
  const needsRebuild = !columnExists('audio_cache_files', 'id');
  if (needsRebuild) {
    db.exec(`
      BEGIN;

      -- Step 1: add language to the old table if not already there
      -- (handles installs that never ran the v0.33.162 migration)
      ${columnExists('audio_cache_files', 'language') ? '' :
        "ALTER TABLE audio_cache_files ADD COLUMN language TEXT NOT NULL DEFAULT 'default';"}

      -- Step 2: backfill NULLs
      UPDATE audio_cache_files SET language = 'default' WHERE language IS NULL OR language = '';

      -- Step 3: create new schema alongside the old one
      CREATE TABLE audio_cache_files_new (
        id         INTEGER PRIMARY KEY AUTOINCREMENT,
        hash       TEXT NOT NULL,              -- SHA-256, hex, 64 chars
        media_name TEXT,                       -- FPP filename
        size_bytes INTEGER,
        mime_type  TEXT,
        language   TEXT NOT NULL DEFAULT 'default',
        cached_at  DATETIME DEFAULT CURRENT_TIMESTAMP,
        UNIQUE(media_name, language)           -- one row per file+language combo
      );

      -- Step 4: copy existing rows
      INSERT INTO audio_cache_files_new (hash, media_name, size_bytes, mime_type, language, cached_at)
        SELECT hash, media_name, size_bytes, mime_type,
               COALESCE(language, 'default'), cached_at
        FROM audio_cache_files;

      -- Step 5: swap
      DROP TABLE audio_cache_files;
      ALTER TABLE audio_cache_files_new RENAME TO audio_cache_files;

      COMMIT;
    `);
    console.log('[migration] audio_cache_files rebuilt with (media_name, language) unique constraint');
  } else if (!columnExists('audio_cache_files', 'language')) {
    // Edge case: id column exists (somehow) but language doesn't
    db.exec(`ALTER TABLE audio_cache_files ADD COLUMN language TEXT NOT NULL DEFAULT 'default'`);
    db.exec(`UPDATE audio_cache_files SET language = 'default' WHERE language IS NULL`);
  }
}
...
function setNowPlaying(sequenceName, secondsPlayed) {
  ...
  if (isNewSequence) {
    // New track — anchor started_at based on the reported position.
    // Use JS Date.now() for millisecond precision — SQLite's CURRENT_TIMESTAMP
    // only has 1-second resolution which causes up to 1000ms sync error.
    const nowMs = Date.now();
    const startedAtMs = nowMs - Math.round(offset * 1000);
    const startedAt = new Date(startedAtMs).toISOString().replace('T', ' ').replace('Z', '');
    db.prepare(`
      UPDATE now_playing
      SET sequence_name = ?,
          started_at = ?,
...
function setBaselineNext(sequenceName) {
  db.prepare(`
    UPDATE now_playing
    SET baseline_next_sequence_name = ?, last_updated = CURRENT_TIMESTAMP
    WHERE id = 1
  `).run(sequenceName);
}
...
//   3. baseline_next_sequence_name — main-playlist return point saved when a winner/
//      request interrupted; valid until that interrupting song finishes
//   4. FPP-reported next (now_playing.next_sequence_name) — live scheduler state
//   5. Fallback → next visible non-PSA song by sort_order (FPP playlist position)
function getNextUp(cfg, nowPlayingName) {
  ...
  // Tier 3: while an interrupting voted/jukeboxed song plays, FPP reports the
  // next song in the voting playlist — not the main playlist we'll return to.
  // Use the baseline (saved at handoff) to show the correct main-playlist return
  // point instead.
  const npBaseline = db.prepare('SELECT baseline_next_sequence_name FROM now_playing WHERE id = 1').get();
  if (npBaseline && npBaseline.baseline_next_sequence_name) return npBaseline.baseline_next_sequence_name;

  // Tier 4: FPP's live-reported next. Correct when no interruption is active.
  // Use FPP's own live "next" report before falling back to sort_order.
  // next_sequence_name is set by POST /api/plugin/next as FPP transitions
  // between songs, so it reflects FPP's actual scheduler state rather than
  // the DB sort_order which was snapshotted at the last playlist sync.
  const npNext = db.prepare('SELECT next_sequence_name FROM now_playing WHERE id = 1').get();
  if (npNext && npNext.next_sequence_name) return npNext.next_sequence_name;
...
function popNextQueuedRequest() {
  const next = db.prepare(`
    ...
    WHERE q.played = 0
      AND q.handed_off_at IS NULL
      AND datetime(q.requested_at, '+120 minutes') > datetime('now')
...
// Cleanup un-handed requests that are too old to serve — e.g. requests that
// arrived during a plugin restart and were never popped. Requests older than
// 2 hours are silently expired so they don't resurface in a later session.
// popNextQueuedRequest also filters these inline as a belt-and-suspenders guard.
function cleanupStaleRequests(olderThanMinutes = 120) {
  db.prepare(`
    UPDATE jukebox_queue
    SET played = 1, played_at = CURRENT_TIMESTAMP
    WHERE played = 0
      AND handed_off_at IS NULL
      AND datetime(requested_at, '+' || ? || ' minutes') < datetime('now')
...
// ============================================================
// Race mode helpers (v0.33.155+)
// ============================================================

// Add a tap for a sequence. No uniqueness check — same viewer can tap many times.
function addRaceTap(sequenceName, viewerToken) {
  db.prepare(`INSERT INTO race_taps (sequence_name, viewer_token) VALUES (?, ?)`)
    .run(sequenceName, viewerToken);
}

// Get tap counts for all sequences, sorted descending.
function getRaceTapCounts() {
  return db.prepare(`
    SELECT sequence_name, COUNT(*) AS count
    FROM race_taps
    GROUP BY sequence_name
    ORDER BY count DESC
  `).all();
...
// Get tap count for a single sequence.
function getRaceTapCount(sequenceName) {
  const row = db.prepare(`SELECT COUNT(*) AS count FROM race_taps WHERE sequence_name = ?`)
    .get(sequenceName);
  return row ? row.count : 0;
}

// Clear all taps (called at race start/reset).
function resetRaceTaps() {
  db.prepare(`DELETE FROM race_taps`).run();
}

// Get the sequence_name currently leading (most taps). Returns null if no taps.
